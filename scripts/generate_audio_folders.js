...
console.log("Generating audio folder structure under:", publicAudioDir);

if (!fs.existsSync(publicAudioDir)) {
  fs.mkdirSync(publicAudioDir, { recursive: true });
}

Object.entries(bookChaptersMap).forEach(([bookCode, numChapters]) => {
  const bookDir = path.join(publicAudioDir, bookCode);
  if (!fs.existsSync(bookDir)) {
    fs.mkdirSync(bookDir, { recursive: true });
  }

  for (let c = 1; c <= numChapters; c++) {
    const chapterDir = path.join(bookDir, String(c));
...
console.log("Successfully generated all audio folders!");
